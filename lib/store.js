...
const insertStmt = db.prepare(
  `INSERT INTO docs (id, user_id, content, theme, title, passcode_enc, created_at, updated_at)
   VALUES (@id, @user_id, @content, @theme, @title, @passcode_enc, @ts, @ts)`
);
...
const listStmt = db.prepare(
  `SELECT id, slug, title, theme, created_at, updated_at, views,
          (passcode_enc IS NOT NULL) AS has_passcode
     FROM docs WHERE user_id = ? ORDER BY updated_at DESC`
);
const updateStmt = db.prepare(
  `UPDATE docs SET content = @content, theme = @theme, title = @title, updated_at = @ts
...
export function createDoc({ content, theme, title, userId = null, passcode = null }) {
  const id = nanoid();
  insertStmt.run({
    id,
    user_id: userId,
...
export function updateDoc({ id, userId, content, theme, title }) {
